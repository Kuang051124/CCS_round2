 ...
 *
 * 基准 (counts/s, 基于实测 96%占空比≈5000ct/s):
 *   快速 ≈28%占空比  中速 ≈14%占空比  慢速 ≈10%占空比
 * =================================================================== */
...
/* A车 — 5段 */
T1Param t4_AO  = { 350.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 400.0f, -400.0f }; /* A→O 慢速DELAY */
T1Param t4_OB  = { 350.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 400.0f, -400.0f }; /* O→B 慢速DELAY */
T1Param t4_ABC = { 600.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 200.0f, -200.0f }; /* BC弧 正常巡线 */
T1Param t4_ACD = { 1000.0f, 800.0f, 40.0f, 20.0f, 150.0f, -150.0f, 750.0f, -750.0f }; /* C→D 快速直   */
T1Param t4_ADA = { 1000.0f, 800.0f, 40.0f, 20.0f, 150.0f, -150.0f, 750.0f, -750.0f }; /* DA弧 快速回A  */
...
T1Param t4_BAB   = { 1000.0f, 800.0f, 40.0f, 20.0f, 150.0f, -150.0f, 750.0f, -750.0f }; /* A→B 快速超A  */
T1Param t4_BBC   = {  600.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 400.0f, -400.0f }; /* BC弧 正常巡线 */
T1Param t4_CSTOP = {    0.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 400.0f, -400.0f }; /* C点停车 spd=0 */
T1Param t4_BCO   = {  350.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 400.0f, -400.0f }; /* C→O 慢速DELAY */
T1Param t4_BOD   = {  350.0f, 600.0f, 30.0f, 20.0f, 100.0f, -100.0f, 400.0f, -400.0f }; /* O→D 慢速DELAY */
...
uint8_t Task4B_Tick(uint8_t key)
{
    ...
    /* ==== OLED 状态 ==== */
    {
        ...
        {
            int32_t t = 0;
            if (seg == 3)      t = T4_STOP_MS;    /* 显示等待ms */
            else if (seg == 4) t = T4_ENC_CO;
            else if (seg == 6) t = T4_ENC_DSTOP;
            sprintf(buf, "Avg:%5ld T:%d", (long)avg, t);
        }
        OLED_ShowString(0, 4, (uint8_t *)buf, 8);
        sprintf(buf, "S%d S:%.0f A:%.0f", seg, tk_param->spd_straight, tk_param->spd_arc);
